...
- **Thread-mode dumps.** `CONFIG_DEBUG_COREDUMP_MEMORY_DUMP_THREADS` dumps thread stacks and metadata instead of Zephyr's default linker-defined RAM image. The default dumps the full ~187 KiB image, most of which is `.bss`, `.data`, and heap buffers that are irrelevant to a fault. Thread mode keeps the stacks and metadata of all threads (plus the faulting context's registers), which stays far smaller than the 256 KiB partition.
