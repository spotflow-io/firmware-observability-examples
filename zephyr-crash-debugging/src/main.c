...
static void button_pressed(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
	...
	LOG_WRN("User button pressed. Arming reproducible crash path.");
...
int main(void)
{
	LOG_INF("Zephyr crash debugging example starting");
	LOG_INF("This demo shows how Spotflow AI Crash Analysis identifies the root cause of a Zephyr RTOS crash.");
	...
	rc = sensor_node_init();
	if (rc < 0) {
		LOG_ERR("Failed to initialize sensor node: %d", rc);
		return rc;
	}
