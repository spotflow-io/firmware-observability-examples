...
/* Compatibility macro: evaluates to 1 if the Zephyr version is >= major.minor */
#ifndef SPOTFLOW_ZEPHYR_VERSION_GE
...
int connect_to_wifi(void)
{
	...
	if (k_sem_take(&wifi_connected_sem, K_SECONDS(30)) != 0) {
		LOG_ERR("Timed out waiting for Wi-Fi connection to %s", WIFI_SSID);
		return -ETIMEDOUT;
	}
